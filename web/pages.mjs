...
export const pages = [
      ...
      <main id="main-content">
        <section class="hero shell section-pad">
          <div class="hero-copy">
            ...
            <h1 data-program-question>Can we shorten the distance to Mars—without changing the traveler?</h1>
            ...
            <p class="microcopy"><strong>Where we are today:</strong> two flat calibrations and one established curved-spacetime calibration pass. No shortcut, device, or route to Mars has been found.</p>
          </div>
        ...
        <section class="status-band" aria-label="Current project status">
          <div class="shell metric-grid">
            <div class="metric"><span data-status="candidateCount">3</span><small>known-answer examples</small></div>
            <div class="metric"><span data-status="checkCount">43</span><small>scoped checks passed</small></div>
        ...
        <section class="shell section-pad boundary-grid">
          <article class="boundary-card positive"><p class="eyebrow">What exists today</p><h2>The measuring tool now recognizes flat and curved known answers.</h2><ul class="clean-list"><li>Two flat-spacetime calibrations, including one changed coordinate scale.</li><li>One established Schwarzschild exterior calibration with position-dependent curvature.</li><li>Forty-three scoped checks recover all three expected answers.</li><li>Separate in-repository solver paths agree within their declared scopes.</li><li>One hundred practice cases for the research workflow.</li></ul><a class="button secondary" href="/lab/">Compare the known-answer tests</a></article>
        ...
        <section class="section-surface" id="mars-benchmark">
          <div class="shell section-pad benchmark-layout">
            ...
            <div class="benchmark-card"><div><span>Goal</span><strong data-program-summary="goal">Reduce the effective Earth-to-Mars journey.</strong></div><div><span>Safety</span><strong data-program-summary="safety">Keep local matter intact and forces survivable.</strong></div><div><span>Physics</span><strong data-program-summary="physics">Satisfy the equations, energy limits, stability, and cause and effect.</strong></div><div><span>Proof</span><strong data-program-summary="proof">Let independent teams recover the same result.</strong></div><div><span>Current position</span><strong data-program-summary="current_position">Three calibration tests work; no travel candidate has been evaluated.</strong></div></div>
          </div>
        </section>
  ...
  },
  {
    ...
    description: 'Inspect two flat-spacetime calibrations and the first established curved Schwarzschild exterior benchmark, including exact limits and open review needs.',
    content: `
      <main id="main-content" data-page="lab">
        <section class="shell page-hero section-pad compact"><div><p class="eyebrow">Test the measuring tools</p><h1>Can the system recover flat space, a coordinate change, and one known curved spacetime?</h1><p class="hero-lede">The first two tests calibrate flat spacetime. The third checks the established curved exterior around a spherical non-rotating mass, including where its coordinate chart is not allowed.</p></div><div class="page-status"><span class="status-dot"></span><strong data-lab-status>Loading result…</strong><small>All three examples are calibration tests, not travel proposals.</small></div></section>
        ...
        <section class="section-surface" id="benchmark-ladder" data-benchmark-ladder>
          <div class="shell section-pad">
            ...
            <div class="boundary-grid benchmark-boundary"><article class="boundary-card positive"><p class="eyebrow">What passing proves</p><h3>One coordinate conversion works exactly.</h3><p>The primary validator passed all fourteen checks. A separate arithmetic path agrees on all thirteen checks it implements; full schema validation remains primary-only. The corrected implementation and provenance passed independent re-review.</p></article><article class="boundary-card caution"><p class="eyebrow">What passing does not prove</p><h3>This is not a general solver.</h3><p>The checker cannot infer unknown transformations, handle nonlinear coordinate maps, test arbitrary curved geometries, calculate a trip, or support a transportation claim. Implementation approval is not an outside scientific reproduction.</p></article></div>
            <div class="hero-actions"><a class="button secondary" href="/data/benchmark-000002-passport.json">Read the preregistered passport</a><a class="button secondary" href="/data/result-000002.json">Inspect all fourteen checks</a><a class="button secondary" href="/data/crosscheck-000002.json">Inspect the separate comparison</a><a class="button secondary" href="/data/review-000002.json">Read the approval record</a></div>
          </div>
        </section>
        <section class="shell section-pad" id="curved-benchmark" data-curved-benchmark>
          <div class="section-intro"><p class="eyebrow">Benchmark 3 of the ladder</p><h2>Known curvature, strict boundaries.</h2><p class="section-lede">Schwarzschild spacetime is the standard exterior geometry around an ideal spherical, non-rotating mass. Unlike the flat examples, its metric changes with position and its curvature is not zero.</p></div>
          <div class="benchmark-verdict panel">
            <div><p class="eyebrow">Preregistered result</p><h3>Can the checker recover a real curved known answer?</h3><p>Yes, inside one restricted coordinate chart: mass <code>M &gt; 0</code>, radius <code>r &gt; 2M</code>, and <code>0 &lt; theta &lt; pi</code>. The horizon boundary, central singularity, and spherical-coordinate axes are excluded.</p><code>sha256:d2b3f4a2ad15a7ade137e4aaf4166bdefab3ff33a3a2a27e5ca232c5bfb43458</code></div>
            <dl class="benchmark-facts"><div><dt>Primary result</dt><dd>17 passed / 0 failed</dd></div><div><dt>Separate solver</dt><dd>EinsteinPy matched 8/8 declared comparisons</dd></div><div><dt>Scientific review</dt><dd>REQUESTED</dd></div><div><dt>Outside signed reproduction</dt><dd>0</dd></div></dl>
          </div>
          <div class="boundary-grid benchmark-boundary"><article class="boundary-card positive"><p class="eyebrow">What passing shows</p><h3>One established curved profile is calculated correctly.</h3><p>The bound SymPy path recovered the metric inverse, determinant, connection, nonzero Riemann curvature, zero exterior Ricci and Einstein tensors, and Kretschmann scalar <code>48M²/r⁶</code>. EinsteinPy produced a matching internal comparison.</p></article><article class="boundary-card caution"><p class="eyebrow">What remains open</p><h3>This does not prove the Mars thesis.</h3><p>No source reconstruction, horizon crossing, geodesic trip, stability, causality, traveler safety, engineering, or novel geometry was tested. The second solver used the same owner, machine, credentials, and Codex workflow and is unsigned.</p></article></div>
          <div class="hero-actions"><a class="button secondary" href="/data/candidate-000003.json">Read Candidate 000003</a><a class="button secondary" href="/data/benchmark-000003-passport.json">Read its passport</a><a class="button secondary" href="/data/result-000003.json">Inspect all 17 checks</a><a class="button secondary" href="/data/crosscheck-000003.json">Inspect the EinsteinPy comparison</a><a class="button secondary" href="/prompts/CANDIDATE_000003_EXTERNAL_REVIEW.md">Send the external review prompt</a></div>
        </section>
      </main>`,
  },
  {
  ...
  },
  {
    ...
    content: `
      <main id="main-content" data-page="contribute">
        ...
        <section class="shell section-pad-top"><div class="contribution-grid program-tasks"><article><span>ASK</span><h2>Make one question testable</h2><p>Choose one research gate and state what evidence would answer or reject the question.</p><a data-repository-issue="question" href="/CONTRIBUTING.md">Ask a bounded question</a></article><article><span>PROPOSE</span><h2>Draft one candidate</h2><p>Declare one exact geometry, its assumptions, expected source, claim, and failure conditions.</p><a data-repository-issue="candidate" href="/CONTRIBUTING.md">Open candidate form</a></article><article><span>CHALLENGE</span><h2>Try to break one claim</h2><p>Identify a hidden assumption, bad equation, missing limit, convention error, or counterexample.</p><a data-repository-issue="challenge" href="/CONTRIBUTING.md">Open challenge form</a></article><article><span>REPRODUCE</span><h2>Repeat one result</h2><p>Use a separate environment, compare the scientific digest, and publish every difference.</p><a data-repository-issue="reproduction" href="#reproduce">Open reproduction form</a></article><article><span>BUILD</span><h2>Add one bounded check</h2><p>Add a deterministic validator with tests, limitations, negative cases, and a reproducible command.</p><a data-repository-issue="build" href="/CONTRIBUTING.md">Open build guidance</a></article><article><span>EXPLAIN</span><h2>Explain one result</h2><p>Make an artifact understandable without changing its status, limitations, or source evidence.</p><a data-repository-issue="explain" href="/CONTRIBUTING.md">Open explanation guidance</a></article></div></section>
        <section class="section-surface" id="research-gates"><div class="shell section-pad"><div class="section-intro"><p class="eyebrow">The shared research contract</p><h2>Eight questions stand between the idea and a serious result.</h2><p class="section-lede">The site reads these gates from one versioned file. A gate can pass only when its named evidence exists; missing capabilities remain visible.</p></div><div class="gates-grid" data-program-gates><article><span>Loading</span><h3>Research gates</h3><p>Reading the current program contract…</p></article></div></div></section>
...
npm run build</code></pre></div></div></section>
        ...
        <section class="section-surface" id="agent-prompts"><div class="shell section-pad"><div class="section-intro"><p class="eyebrow">Agent build prompts</p><h2>Give coding and research agents a bounded role, not a vague command to “solve it.”</h2><p class="section-lede">The repository contains the complete ordered Mars research program plus bounded prompts for orchestration, building, science, skepticism, reproduction, security, release, and explanation.</p></div><div class="prompt-links" data-prompt-links><a href="/prompts/MARS_RESEARCH_PROGRAM.md">Complete Mars program</a><a href="/prompts/ORCHESTRATOR_AGENT.md">Orchestrator</a><a href="/prompts/BUILD_AGENT.md">Build agent</a><a href="/prompts/SCIENCE_AGENT.md">Science agent</a><a href="/prompts/SKEPTIC_AGENT.md">Skeptic</a><a href="/prompts/PHASE_1_IMPLEMENTATION.md">Phase 1 build</a></div></div></section>
