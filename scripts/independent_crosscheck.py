...
from independent.minkowski_crosscheck import run_crosscheck


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the separate standard-library Minkowski cross-check.")
    parser.add_argument("--write", action="store_true")
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    result = run_crosscheck()
    if args.write:
        output = ROOT / "artifacts" / "reproductions" / "CANDIDATE-000001.crosscheck.json"
